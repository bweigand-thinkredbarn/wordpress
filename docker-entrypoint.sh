...
EOPHP
			# leaving wp-config-sample open can break some plugins
			# and can prevent some docker configs like linking between
			# container storage before build
			close('wp-config-sample.php') >> log.txt
			#
			#
